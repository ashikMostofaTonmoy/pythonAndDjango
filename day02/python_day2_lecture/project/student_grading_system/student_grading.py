...
# Global variables
students = {}  # Dictionary to store student data: {student_id: {'name': name, 'grades': {subject: [grades]}}}
...
def add_student():
    ...
    # Get student ID
    while True:
        student_id = get_valid_input("Enter student ID: ", is_valid_student_id, "Student ID cannot be empty.")
    ...
    # Get student name
    name = get_valid_input("Enter student name: ", is_valid_name, "Name cannot be empty.")
...
def record_grade():
    ...
    # Get subject
    subject = get_valid_input("Enter subject: ", is_valid_subject, "Subject cannot be empty.")
    ...
    # Get grade
    grade = float(get_valid_input("Enter grade (0-100): ", is_valid_grade, "Grade must be a number between 0 and 100."))
...
def view_student_grades():
    ...
    if not grades:
        ...
        print("\n{:<15} {:<10} {:<10} {:<10}".format("Subject", "Average", "Highest", "Lowest"))
        print("-" * 45)
        ...
        for subject, subject_grades in grades.items():
            if subject_grades:
                ...
                print("{:<15} {:<10.2f} {:<10.2f} {:<10.2f}".format(subject, avg, highest, lowest))
                print(f"  All grades: {', '.join(str(g) for g in subject_grades)}")
...
def class_summary():
    ...
    if all_grades:
        ...
        for i, (student_id, avg) in enumerate(sorted_students[:3], 1):
            ...
            print(f"{i}. {name} (ID: {student_id}) - Average: {avg:.2f}, Grade: {get_letter_grade(avg)}")
        
        # Display all students
        print("\nAll Students:")
        print("\n{:<5} {:<15} {:<10} {:<10}".format("ID", "Name", "Average", "Grade"))
        print("-" * 40)
        ...
        for student_id, data in students.items():
            ...
            if student_grades:
                ...
                print("{:<5} {:<15} {:<10.2f} {:<10}".format(student_id, name, avg, grade))
            else:
                print("{:<5} {:<15} {:<10} {:<10}".format(student_id, name, "N/A", "N/A"))
    else:
...
def subject_summary():
    ...
    if subject_grades:
        ...
        sorted_students = sorted(subject_grades.items(), key=lambda x: x[1], reverse=True)
...
def export_data():
    ...
    with open(summary_filename, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Student ID", "Name", "Overall Average", "Letter Grade"])
            ...
            if header == ["Student ID", "Name"]:
                ...
                for row in reader:
                    if len(row) >= 4:
                        ...
                        try:
                            grade_value = float(grade)
                            students[student_id]['grades'][subject].append(grade_value)
                        except ValueError:
                            print(f"Invalid grade value '{grade}' for student {name}. Skipping.")
